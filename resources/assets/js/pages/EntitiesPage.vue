...
import ModalGoogleForm from "../components/ModalGoogleForm";
...
export default {
  ...
  created() {
    ...
    Promise.all([
      ...
      .then(
        ([responseClassification, responseEntities, responseFacetGroups]) => {
  ...
  methods: {
    ...mapMutations(["resetFilters", "loading", "loaded"]),
    filter: function () {
      const slug = this.$route.params.classification;
      const url = this.$store.getters.getUrl;
      ...
      axios
        .post(
          url + "/api/classifications/" + slug + "/entities",
    ...
    <section class="hero">
      <div class="hero-body">
        <breadcrumb
          :items="[
            ...
            {
              url: '/app/classificacoes/' + classification.slug,
        ...
        <div class="row content">
          ...
          <p class="has-text-justified">
            Nesta página são apresentados os elementos que compõe a
            classificação acessada.
          </p>
        </div>
        ...
        <div class="row content" v-if="filteredEntities.length">
          <div class="columns is-multiline">
            <div class="column is-12">
              <h2 class="subtitle">
                <b-icon icon="format-list-bulleted"></b-icon> Registros
                encontrados
              </h2>
            </div>
            <div
              class="column is-4 is-6-tablet is-12-mobile"
        ...
        <div class="row content" v-else>
          <p v-if="$store.getters.getFiltersValues.length">
      ...
      >
        <modal-form
          ...
          :horizontal-data="
            facetGroups.filter((elem) => elem.layout === 'horizontal')
          "
          :vertical-data="
            facetGroups.filter((elem) => elem.layout === 'vertical')
          "
          @filter="filter"
          @reset="reset"
        ></modal-form>
