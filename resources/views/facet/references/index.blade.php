...
@section('content')
    <div class="columns has-background-white">
        <div class="column is-12">
            ...
            <form id="form" action="{{ route('facets.references', ['classificationId' => $classificationId, 'facetId' => (int) $facetId]) }}" method="POST">
                @csrf
            ...
            <table class="table is-fullwidth is-striped is-hoverable is-bordered">
                ...
                <tbody>
                @foreach ($facet->references as $reference)
                    <tr>
                        <td>{{ $reference->pivot->code }}</td>
                        <td>{{ $reference->description }}</td>
                        <td class="has-text-centered">
                            <a class="button is-small delete-button"
                               data-target="modal" aria-haspopup="true" data-id="{{ $reference->id }}" data-classification="{{ $facetId }}"
                ...
                <tr>
                    <td colspan="5">
                        @if (count($facet->references) <= 0) Nenhuma referência associada a esta faceta @endif
                        &nbsp;
                    </td>
                </tr>
                </tfoot>
    ...
    <div class="modal" id="modal">
        ...
        <div class="modal-card">
            ...
            <footer class="modal-card-foot">
                <button class="button is-danger modal-confirm" data-id="" data-classification="" @click="attemptDetachFacetReference()">{{ __('Remover') }}</button>
